...
public class PrefabBrushEditor : EditorWindow
{
    ...
    const string _defaultSaveFilePath = "Assets\\PrefabBrush\\Editor\\Data\\SaveData.txt";
    const string _brushSaveFilePath = "Assets\\PrefabBrush\\Editor\\Data\\";
    ...
    void OnDestroy()
    {
        ...
        if (!File.Exists(_defaultSaveFilePath))
        {
            File.Create(_defaultSaveFilePath).Dispose();
        }
    ...
    void OnGUI()
    {
        ...
        if (_enabled)
        {
            ...
            GUILayout.Label("[Enabled] Toggle: Ctrl + E", style);
        }
        else
        {
            var style = new GUIStyle(EditorStyles.boldLabel);
            style.normal.textColor = Color.red;
            GUILayout.Label("[Disabled] Toggle: Ctrl + E", style);
        }
    ...
    {
        // This logic helps bypass the inconsistencies in Unity's GUI
        ...
        // Ctrl + E == Toggle Enabled Flag
        if (Event.current.keyCode == KeyCode.E && Event.current.type == EventType.KeyUp && _ctrlDown)
        {
            _enabled = !_enabled;
            Repaint();
        }

        // If we're on a surface, and the tool is in use
        if (on_surface && _enabled)
        {
            // If mouse button 1 is the current button
            ...
            // Get our tool's control ID
            int control_ID = GUIUtility.GetControlID(_editorHash, FocusType.Passive);

            // This allows us to intercept all input events
    ...
    void PlacePrefab()
    {
        ...
        if (_prefabMenu.HasPrefab())
        {
            ...
            // UndoRedo
            Undo.RegisterCreatedObjectUndo(obj, "Prefab Brush Created Object");
    ...
    void PlacePrecisionPrefab()
    {
        ...
        if (_prefabMenu.HasPrefab())
        {
            ...
            obj.transform.position = _brush.pos;
            obj.transform.rotation = rotation;
    ...
    void ItteratePrecisionPrefab()
    {
        if (_prefabMenu.HasPrefab())
        {
            ...
            GameObject newObj = PrefabUtility.InstantiatePrefab(prefab.gameObject) as GameObject;
            newObj.transform.position = _precision.transform.position;
            newObj.transform.rotation = _precision.transform.rotation;

            // UndoRedo
            Undo.RegisterCreatedObjectUndo(newObj, "Prefab Brush Created Object");
